...
**包路径**：`com.formacraft.common.generation.component`

**核心接口**：
- `ComponentGenerator` - 组件生成器接口
- `ComponentGeneratorRegistry` - 生成器注册表
...
**调用链**：
```
LlmPlan → ComponentPlanCompiler → ComponentGeneratorRegistry → ComponentGenerator → List<BlockPatch>
```
...
2. 在 `ComponentGeneratorRegistry` 中注册
