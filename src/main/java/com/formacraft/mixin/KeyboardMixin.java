...
public class KeyboardMixin {
    ...
    private void onKey(long window, int action, KeyInput keyInput, CallbackInfo ci) {
        ...
        // 释放：不转发、不 cancel，但需要清理“吞 ESC 直到释放”的标记
        if (action == 0) {
            if (isEscape) {
                InputRouter.clearConsumeEscapeUntilRelease();
            }
            return;
        }

        // UI 关闭时：默认不处理，让游戏使用原版行为
        // 但若正在吞 ESC（UI 刚刚被 ESC 关闭），则仍需拦截 ESC 的 repeat，避免打开暂停菜单。
        if (!FormacraftUIState.isOpen) {
            if (isEscape && InputRouter.isConsumingEscapeUntilRelease()) {
                ci.cancel();
            }
            return;
        }
