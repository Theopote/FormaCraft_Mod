...
/**
 * Parametric radial terrace hall builder (typology: {@code radial_terrace_hall}).
 * 三层圆形台基 + 圆殿 + 三重檐（天坛祈年殿意象）。
 */
public final class RadialTerraceHallBuilder {

    public static final String TYPOLOGY_ID = "radial_terrace_hall";

    private static final FcaLog LOG = FcaLog.of("RadialTerraceHallBuilder");

    private RadialTerraceHallBuilder() {}
    ...
    private static GeneratedStructure generateFromSpec(BuildingSpec spec, BlockPos origin, ServerWorld world) {
        ...
        BlockState base = blockOrDefault(world, getStringExtra(spec, "baseBlock", "minecraft:smooth_quartz"), Blocks.SMOOTH_QUARTZ.getDefaultState());
        BlockState stair = blockOrDefault(world, getStringExtra(spec, "stairBlock", "minecraft:quartz_stairs"), Blocks.QUARTZ_STAIRS.getDefaultState());
        BlockState trim = blockOrDefault(world, getStringExtra(spec, "trimBlock", "minecraft:quartz_slab"), Blocks.QUARTZ_SLAB.getDefaultState());
        BlockState pillar = blockOrDefault(world, getStringExtra(spec, "pillarBlock", "minecraft:white_concrete"), Blocks.WHITE_CONCRETE.getDefaultState());
        BlockState wall = blockOrDefault(world, getStringExtra(spec, "wallBlock", "minecraft:white_concrete"), Blocks.WHITE_CONCRETE.getDefaultState());
        BlockState roof = blockOrDefault(world, getStringExtra(spec, "roofBlock", "minecraft:cyan_terracotta"), Blocks.CYAN_TERRACOTTA.getDefaultState());
        BlockState accent = blockOrDefault(world, getStringExtra(spec, "accentBlock", "minecraft:yellow_terracotta"), Blocks.YELLOW_TERRACOTTA.getDefaultState());
        ...
        String ref = getStringExtra(spec, "reference_landmark", "");
        String desc = String.format(
                "radial_terrace_hall (baseR=%d, hallR=%d, tiers=%d, ref=%s, score=%.2f[shape=%.2f,ratio=%.2f,sig=%.2f])",
                baseRadius, hallRadius, tiers, ref.isBlank() ? "-" : ref, overall, shapeScore, ratioScore, signatureScore
        );
    ...
    private static BlockState blockOrDefault(ServerWorld world, String id, BlockState defaultState) {
