...
import com.formacraft.server.generation.structure.HouseRoofGenerator;
...
/**
 * Parametric tailiang timber hall builder (typology: {@code tailiang_timber_hall}).
 */
public final class TailiangTimberHallBuilder {

    public static final String TYPOLOGY_ID = "tailiang_timber_hall";

    private TailiangTimberHallBuilder() {}
    ...
    public static GeneratedStructure generate(
        ...
        Direction entrance = specHint != null
        ...
        int subDepth = subEaves ? p.intVal("subEavesDepth", 3, 2, 5) : 0;
        ...
        String desc = String.format(
                "tailiang_timber_hall (%d×%d bays, w=%d d=%d, subEaves=%s, ref=%s)",
                baysX, baysZ, w, d, subEaves, ref.isBlank() ? "-" : ref
        );
    ...
    private static void addLatticeWindows(List<PlannedBlock> blocks, BlockPos origin, Direction entrance,
        ...
        for (int x = spacing; x < w; x += spacing) {
            ChineseTypologyDetailUtil.addLatticeWindow(
                    (pos, state) -> put(blocks, origin, entrance, pos.getX(), pos.getY(), pos.getZ(), state),
    ...
    private static void addPuzuoAtColumns(List<PlannedBlock> blocks, BlockPos origin, Direction entrance,
        ...
        for (int[] col : ChineseTypologyDetailUtil.perimeterColumnPositions(x0, z0, x1, z1, spacing)) {
            int x = col[0];
            int z = col[1];
            boolean corner = ChineseTypologyDetailUtil.isCornerColumn(x, z, x0, z0, x1, z1);
            Direction out = ChineseTypologyDetailUtil.outwardFromRectEdge(x, z, x0, z0, x1, z1);
            Direction along = ChineseTypologyDetailUtil.alongWallFromEdge(x, z, x0, z0, x1, z1);
            ChineseTypologyDetailUtil.PuzuoProfile profile =
                    ChineseTypologyDetailUtil.resolvePuzuoProfile(subEaves, corner);
            ChineseTypologyDetailUtil.addPuzuoZoning(
                    (pos, state) -> put(blocks, origin, entrance, pos.getX(), pos.getY(), pos.getZ(), state),
